...
import {
  ...
  type WorkflowTurnItem as ReadThreadTurnItem,
...
export function readLatestSessionLog(
  target?: SessionLogTarget,
): SessionLogSnapshot | null {
...
export function parseSessionLog(source: string): SessionLogSnapshot {
  ...
  const meta =
    events.find((event) => event.type === "session_meta")?.payload ?? {};
  ...
  for (const event of events) {
    ...
    if (event.type === "event_msg" && payload.type === "task_started") {
      ...
      currentStartedAt =
        toMs(payload.started_at) ?? toMs(event.timestamp) ?? Date.now();
      current = null;
      continue;
    }
    ...
    if (event.type === "event_msg" && payload.type === "user_message") {
      ...
      const user = cleanUserMessage(
        userContentText(userContent) || stringValue(payload.message),
      );
      current = {
    ...
    if (
      ...
      const userContent = userContentFromPayload(payload);
      const user = cleanUserMessage(
        userContentText(userContent) || messageContentText(payload.content),
      );
      current = {
    ...
    if (event.type === "event_msg" && payload.type === "agent_message") {
      ...
      if (text) {
        ...
        addAssistantItems(
          `assistant-${messages.length}-${ensureMessage().items.length}`,
          text,
          addItem,
        );
      }
    ...
    if (event.type === "response_item" && payload.type === "plan") {
      addItem({
        id:
          ...
          `plan-${messages.length}-${ensureMessage().items.length}`,
        type: "plan",
    ...
    if (
      ...
      addItem({
        ...
        status: stringValue(payload.status) || "running",
        senderThreadId:
          stringValue(payload.senderThreadId) ||
          stringValue(payload.sender_thread_id) ||
          undefined,
        receiverThreadIds: stringArray(
          payload.receiverThreadIds ?? payload.receiver_thread_ids,
        ),
    ...
    if (event.type === "response_item" && payload.type === "tool_search_call") {
      const id =
        stringValue(payload.call_id) || `tool-search-${calls.size + 1}`;
    ...
    if (
      ...
      const id: string =
        payloadCallId || lastWebSearchCallId || `web-search-${calls.size + 1}`;
      ...
      upsertWebSearchItem(
        ...
        id,
        input,
        stringValue(payload.status) || "running",
      );
      continue;
    }
    ...
    if (event.type === "event_msg" && payload.type === "web_search_end") {
      ...
      const id: string = calls.has(payloadCallId)
        ? payloadCallId
        : lastWebSearchCallId ||
          payloadCallId ||
          `web-search-${calls.size + 1}`;
    ...
    if (
      ...
      ensureMessage().activity = "running";
      upsertImageGenerationItem(
        ensureMessage(),
        id,
        input,
        status,
        output,
        imageGenerationResult(payload),
        imageGenerationSavedPath(payload),
      );
      continue;
    }
    ...
    if (event.type === "response_item" && payload.type === "local_shell_call") {
      const id =
        stringValue(payload.call_id) ||
        stringValue(payload.id) ||
        `local-shell-${calls.size + 1}`;
      const command =
        stringValue(payload.command) ||
        parseArgs(payload.arguments ?? payload.input);
      ...
      if (name === "apply_patch") {
        ...
        addItem({
          ...
          changes: files.map((path) => ({
            path,
            kind: "modified",
            diff: patchText ? { text: patchText } : undefined,
          })),
        });
      }
    ...
    if (event.type === "response_item" && payload.type === "image_view") {
      addItem({
        id:
          stringValue(payload.id) ||
          `image-view-${messages.length}-${ensureMessage().items.length}`,
        type: "imageView",
    ...
    if (event.type === "event_msg" && payload.type === "mcp_tool_call_end") {
      ...
      const tool = calls.get(id);
      const output = compactOutput(
        payload.result || payload.invocation || "completed",
      );
    ...
    if (event.type === "event_msg" && payload.type === "hook_prompt") {
      ...
      addItem({
        ...
        type: "hookPrompt",
        fragmentCount: Array.isArray(fragments)
          ? fragments.length
          : (numberValue(payload.fragment_count) ?? 0),
        fragments,
      });
      continue;
    }
    ...
    if (event.type === "event_msg" && payload.type === "item_completed") {
      ...
      if (stringValue(item?.type).toLowerCase() === "plan") {
        addItem({
          id:
            stringValue(item?.id) ||
            `plan-snapshot-${messages.length}-${ensureMessage().items.length}`,
          type: "plan",
    ...
    if (event.type === "event_msg" && payload.type === "thread_rolled_back") {
      addItem({
        ...
        arguments: {},
        output: textOutput(
          compactOutput({ num_turns: numberValue(payload.num_turns) ?? 1 }),
        ),
        status: "completed",
      });
      continue;
    ...
    if (event.type === "event_msg" && payload.type === "turn_aborted") {
      const message = ensureMessage();
      const completedAt =
        toMs(payload.completed_at) ??
        unixSecondsToMs(payload.completed_at) ??
      ...
      message.durationMs =
        ...
        (message.startedAt
          ? Math.max(0, completedAt - message.startedAt)
          : null);
      addItem({
    ...
    if (
      ...
      (payload.type === "task_failed" || payload.type === "error")
    ) {
      const message = ensureMessage();
      message.status = "failed";
      message.activity = "failed";
      ...
      addItem({
        ...
        arguments: {},
        output: textOutput(
          compactOutput(payload.message ?? payload.error ?? payload),
        ),
        status: "error",
      });
      continue;
    ...
    if (event.type === "event_msg" && payload.type === "task_complete") {
      ...
      if (
        lastMessage &&
        !message.items.some(
          (item) => item.type === "agentMessage" && item.text === lastMessage,
        )
      ) {
        addAssistantItems(
...
function sessionMessagesToReadThread(
  ...
  const chronologicalTurns = messages.map((message) => {
    const items: ReadThreadTurnItem[] = message.items.map(
      workflowItemToReadThreadItem,
    );
    if (
      message.userContent.length &&
  ...
  const preview =
    readThreadPreview(latestTurn) ?? readThreadPreview(firstTurn) ?? "";
...
function readThreadPreview(
  turn: WorkflowReadThreadResponse["turns"][number] | undefined,
): string | undefined {
...
function normalizedUserText(message: WorkflowMessageBlock): string {
  return normalizeForCompare(
    userContentText(message.userContent) || message.user,
  );
}
...
function patchInput(payload: Record<string, unknown>): string {
  ...
  if (typeof args === "string") {
    try {
      ...
      return (
        stringValue(asRecord(parsed)?.patch) ||
        stringValue(asRecord(parsed)?.input) ||
        args
      );
    } catch {
      return args;
...
function toolSearchOutput(payload: Record<string, unknown>): string {
  ...
  return names.length
    ? `Found ${names.length} tools:\n${names.join("\n")}`
    : compactOutput(payload);
}

function toolItemFromCall(
  id: string,
  name: string,
  input: string,
  status: string,
  ...
  if (name === "view_image") {
    const args =
      asRecord(payload.arguments) ?? parseJsonRecord(payload.arguments);
    return {
      type: "imageView",
      id,
      path:
...
function shellFromPayload(
  ...
  const args =
    ...
    parseJsonRecord(payload.input);
  return (
    stringValue(payload.shell) ||
...
function userContentFromPayload(
  payload: Record<string, unknown>,
  ...
  for (const image of arrayRecords(payload.images)) {
    ...
    const path =
      stringValue(image.path) ||
      stringValue(image.file) ||
      stringValue(image.local_path);
...
function userContentPart(value: unknown): WorkflowUserMessageContent | null {
  if (typeof value === "string")
    return value.trim()
      ? { type: "text", text: cleanUserMessage(value) }
      : null;
  ...
  if ((type === "text" || type === "input_text" || !type) && text.trim())
    return { type: "text", text: cleanUserMessage(text) };
  ...
  if (type === "skill") {
    ...
    return name
      ? { type: "skill", name, path: stringValue(record.path) || undefined }
      : null;
  }
  ...
  if (type === "mention") {
    const name = stringValue(record.name) || stringValue(record.title);
    return name
      ? { type: "mention", name, path: stringValue(record.path) || undefined }
      : null;
  }
...
function isOutputItem(
  item: WorkflowStreamItem,
): item is Extract<WorkflowStreamItem, { output?: TextOutput }> {
  return (
...
function completeFileItem(
  ...
  const existing = message.items.find(
    (item) => item.type === "fileChange" && item.id === editingId,
  );
  if (existing?.type === "fileChange") {
    existing.status = "completed";
    existing.changes = (
      files.length ? files : existing.changes.map((change) => change.path)
    ).map((path) => ({
      path,
      kind: "modified",
      diff: patch
        ? { text: patch }
        : existing.changes.find((change) => change.path === path)?.diff,
    }));
    return;
  }
  ...
  if (files.length) {
    message.items.push({
      ...
      changes: files.map((path) => ({
        path,
        kind: "modified",
        diff: patch ? { text: patch } : undefined,
      })),
    });
  }
}

function failFileItem(
  message: WorkflowMessageBlock,
  callId: string,
  files: string[],
  patch = "",
...
function upsertUsageItem(message: WorkflowMessageBlock, output: string): void {
  ...
  const existing = message.items.find(
    (item) => item.type === "dynamicToolCall" && item.id === id,
  );
...
function updateToolOutput(
  ...
  const item = message.items.find(
    (entry) => isOutputItem(entry) && entry.id === id,
  );
  if (item && isOutputItem(item) && !item.output)
    item.output = textOutput(output);
}
...
function updateToolStatus(
  ...
  const item = message.items.find(
    (entry) => "status" in entry && entry.id === id,
  );
...
function upsertWebSearchItem(
  ...
  const existing = message.items.find(
    (item) => item.type === "webSearch" && item.id === id,
  );
...
function upsertImageGenerationItem(
  ...
  const existing = message.items.find(
    (item) => item.type === "imageGeneration" && item.id === id,
  );
...
function parseArgs(args: unknown): string {
  ...
  try {
    ...
    if (record?.title || record?.code)
      return [record.title, record.code].filter(Boolean).join("\n");
