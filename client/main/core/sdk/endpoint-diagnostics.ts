...
export async function diagnoseAnthropicCompatibleEndpoint(
  ...
  if (!baseUrl) return { ok: false, message: "Model endpoint is missing Base URL." };
  if (!apiKey) return { ok: false, message: "Model endpoint is missing API Key." };
  if (!model) return { ok: false, message: "Model endpoint is missing model ID." };
  ...
  try {
    ...
    if (response.ok) {
      return {
        ...
        message: "Diagnostic request succeeded; the endpoint can generate content.",
        body,
      };
    }
    return {
...
function explainEndpointError(status: number, body: string): string {
  ...
  if (status === 401 || status === 403 || /invalid api key|not authorized|token/i.test(compactBody)) {
