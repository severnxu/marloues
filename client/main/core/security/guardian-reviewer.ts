...
import { resolveGatewayTarget } from "../../gateway";
...
async function requestGuardianReview(input: {
  ...
  const target = resolveGatewayTarget(input.baseUrl);
  ...
  const encoded = encodeRequest(target.protocol, request);
  const headers = {
    ...encoded.headers,
  ...
  const response = await fetchWithHardTimeout(
    buildProviderUrl(target.baseUrl, encoded.path),
    {
      method: "POST",
      headers,
