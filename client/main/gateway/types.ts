...
export function detectProtocol(url: string): ProtocolId | null {
  if (url.indexOf("/v1/responses") >= 0 || url.match(/\/responses(?:\?|$)/))
    return "openai-responses";
  if (
    url.indexOf("/v1/chat/completions") >= 0 ||
    url.match(/\/chat\/completions(?:\?|$)/)
  )
    return "openai-chat";
  if (url.indexOf("/v1/messages") >= 0 || url.match(/\/messages(?:\?|$)/))
    return "anthropic";
  return null;
}
