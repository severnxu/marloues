...
export function WorkflowFileChangeRow({ item }: Props) {
  ...
  const label = item.status === 'failed' || item.status === 'error' ? '编辑失败' : running ? '正在编辑' : '已编辑'
  const patchLines = item.changes.flatMap(change => patchPreviewLines(change.diff?.text ?? ''))
  ...
  return (
    <WorkflowActivityRow
      ...
      label={(
        <>
          {label} <span className={files.length === 1 ? 'font-mono text-accent' : 'text-text-subtle'}>{fileChangeTargetLabel(files)}</span>
...
function PatchPreviewCard({ path, lines, rawDiff }: { path: string; lines: DiffLine[]; rawDiff: string }) {
  const [copied, setCopied] = useState(false)
  const diffHostRef = useRef<HTMLDivElement | null>(null)
  const diffThemeType = useThemeStore(state => state.isDark ? 'dark' : 'light')
  ...
  useEffect(() => {
    ...
    const install = () => {
      ...
      let style = shadowRoot.querySelector<HTMLStyleElement>('style[data-marloues-diff-scrollbar]')
      if (!style) {
  ...
  const copyDiff = async () => {
    try {
      await navigator.clipboard?.writeText(rawDiff || lines.map(line => `${line.prefix}${line.text}`).join('\n'))
      setCopied(true)
  ...
  return (
    <div className="workflow-patch-card workflow-diffs-card">
      <div ref={diffHostRef} className="workflow-patch-card-body" data-kind="diffs-view">
        {diffsPatch ? (
          <PatchDiff
            patch={diffsPatch}
            ...
            renderCustomHeader={(fileDiff) => (
              <DiffCustomHeader fileDiff={fileDiff} copied={copied} onCopy={copyDiff} />
            )}
          />
        ) : (
          lines.map((line, index) => (
            <div key={`${line.text}-${index}`} className={`workflow-patch-line ${line.kind}`}>
              {line.prefix}{line.text}
            </div>
          ))
        )}
        {!lines.length ? <div className="workflow-patch-empty">无 diff 预览</div> : null}
      </div>
    </div>
  )
}
    ...
    <div className="workflow-diffs-custom-header">
      <div className="workflow-diffs-custom-title">
        ...
        <span className="workflow-diffs-counts">
          {added ? <span className="workflow-diffs-count workflow-diffs-count-add">+{added}</span> : null}
          {removed ? <span className="workflow-diffs-count workflow-diffs-count-delete">-{removed}</span> : null}
        </span>
      </div>
      <button type="button" className="workflow-patch-copy" onClick={() => void onCopy()} aria-label={copied ? 'Copied diff' : 'Copy diff'} title={copied ? 'Copied' : 'Copy diff'}>
...
function normalizePatchForDiffs(patch: string, fallbackPath: string): string {
  ...
  if (!trimmed) return ''
  if (/^diff --git\s+/m.test(trimmed) && /^@@\s+-\d+/m.test(trimmed)) return patch
