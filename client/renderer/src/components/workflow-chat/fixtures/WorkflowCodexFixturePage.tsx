...
export function WorkflowCodexFixturePage() {
  const fixtureState =
    new URLSearchParams(window.location.search).get("workflowFixtureState") ??
    "all";
  ...
  const showInterrupted = fixtureState === "interrupted";
  ...
  return (
    <main className="workflow-fixture-page">
      <div className="workflow-fixture-shell">
        ...
        {showInterrupted ? (
          <WorkflowTurnList
            workflowMessages={interruptedMessages}
            isStreaming
            stateScopeKey="codex-fixture-interrupted"
            modelName="GPT-5"
            onCopyMessage={() => undefined}
            onFork={() => undefined}
          />
...
export function WorkflowChatShellFixturePage() {
  ...
  const fixtureData =
    new URLSearchParams(window.location.search).get("workflowFixtureData") ??
    "workflow";
  const injectedReadThread = window.__MARLOUES_WORKFLOW_FIXTURE_READ_THREAD__;
  const useReadThread =
    fixtureData === "readThread" ||
    fixtureData === "latestCodexReadThread" ||
    fixtureData === "sessionReadThread";
  ...
  const newestTurn =
    readThread.page.order === "newest_first"
      ? readThread.turns[0]
      : readThread.turns.at(-1);
  const fixtureIsStreaming =
    readThread.thread.status.type === "active" ||
    newestTurn?.status === "running";
...
const runningMessages: WorkflowMessageBlock[] = [
  {
    ...
    userContent: [
      {
        type: "text",
        text: "继续扣，我要让我的 agent 在显示层和 Codex 一模一样",
      },
    ],
    status: "running",
    activity: "running",
    ...
    items: [
      {
        ...
        id: "cmd-read",
        command:
          "Get-Content -Path src/renderer/src/components/workflow-chat/turn-layout.ts -TotalCount 260",
        status: "completed",
        output: {
      ...
      },
      {
        ...
        id: "cmd-folder",
        command:
          "New-Item -ItemType Directory -Path src/renderer/src/components/workflow-chat/fixtures",
        status: "inProgress",
      },
      {
        type: "fileChange",
      ...
      },
      {
        ...
        reason: JSON.stringify({
          file: "src/renderer/src/components/workflow-chat/turn-layout.ts",
          action: "edit",
        }),
        status: "pending",
        timeoutMs: 60_000,
    ...
    items: [
      {
        ...
        id: "cmd-1",
        command:
          'rg -n "workflow-activity-row" src/renderer/src/components/workflow-chat',
        status: "completed",
        output: {
          text: "Output:\nActivityRow.tsx:28: workflow-activity-row",
          truncated: false,
        },
      },
      {
        ...
        changes: [
          {
            ...
            kind: "edit",
            diff: {
              text: "+<InlineDiffStats added={added} removed={removed} />\n-<span />",
              truncated: false,
            },
          },
        ],
...
const interruptedMessages: WorkflowMessageBlock[] = [
  {
    id: "fixture-interrupted",
    user: "先总结当前实现",
    userContent: [{ type: "text", text: "先总结当前实现" }],
    status: "completed",
    activity: "done",
    startedAt: baseTime - 90_000,
    completedAt: baseTime - 30_000,
    durationMs: 60_000,
    ...
    items: [
      {
        ...
        text: "当前实现已经完成消息流梳理，并确认引导会中断正在生成的回复。",
      },
    ],
  },
  {
    id: "fixture-interrupted:steer:guide",
    user: "立即引导：先把中断片段的操作补齐",
    userContent: [{ type: "text", text: "立即引导：先把中断片段的操作补齐" }],
    status: "running",
    activity: "responding",
...
const completedReadThread = workflowMessagesToWorkflowReadThreadResponse(
  completedMessages,
  {
