import type { WorkflowMessageBlock, WorkflowTurnItem } from '../../../../shared/adapters/workflow-messages-to-read-thread'
import { workflowItemIsRunning, workflowShouldShowActivityItem, type WorkflowProcessItem } from './turn-collapse-rules'
import type { WorkflowTurnLayout } from './turn-layout'

export type WorkflowTurnPresentation =
  | { type: 'thinking-empty'; showHeader: false; showThinkingPlaceholder: true; statusLabel: string; thinkingVisible: boolean }
  | { type: 'active-flow'; showHeader: true; showThinkingPlaceholder: false }
  | { type: 'answering'; showHeader: true; showThinkingPlaceholder: boolean; statusLabel?: string; thinkingVisible?: boolean }
  | { type: 'processing-hidden'; showHeader: true; showThinkingPlaceholder: false }
  | { type: 'complete'; showHeader: true; showThinkingPlaceholder: false }
...
export function workflowTurnPresentation(
  ...
  if (message.activity === 'done' || message.status === 'completed') {
    return { type: 'complete', showHeader: true, showThinkingPlaceholder: false }
  }
  ...
  if (layout.leadingFlow.length > 0 || layout.trailingFlow.length > 0) {
    if (hasVisibleRunningWork([...layout.leadingActivityItems, ...layout.trailingActivityItems])) {
      return { type: 'active-flow', showHeader: true, showThinkingPlaceholder: false }
    }
    if (isLastStreaming && message.status === 'running') {
      return { type: 'answering', showHeader: true, showThinkingPlaceholder: true, statusLabel: '正在思考', thinkingVisible: true }
    }
    return { type: 'processing-hidden', showHeader: true, showThinkingPlaceholder: false }
  }
  ...
  if (hasHiddenRunningWork(message.items)) {
    return { type: 'thinking-empty', showHeader: false, showThinkingPlaceholder: true, statusLabel: '正在思考', thinkingVisible: true }
  }
...
function hasHiddenRunningWork(items: WorkflowTurnItem[]): boolean {
  return items.some(item => {
    if (item.type === 'userMessage' || item.type === 'agentMessage') return false
    const processItem = item as WorkflowProcessItem
    return workflowItemIsRunning(processItem) && !workflowShouldShowActivityItem(processItem)
  })
}
...
function hasAnswerGap(layout: WorkflowTurnLayout): boolean {
  ...
  return !hasVisibleRunningWork([...layout.leadingActivityItems, ...layout.trailingActivityItems])
}
