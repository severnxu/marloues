...
export function AssistantTurnHeader({
  ...
  return (
    <div className="workflow-turn-header" data-activity={activity}>
      <div className="workflow-turn-header-row">
        <button
          type="button"
          onClick={() => interactive && onToggle()}
          aria-expanded={interactive ? expanded : undefined}
          data-kind="turn-header"
          className={`workflow-turn-header-button ${interactive ? "is-interactive" : "is-static"} ${tone}`}
        >
          <span className="workflow-turn-status-label">{label}</span>
          {duration ? (
            <span className="workflow-turn-duration">{duration}</span>
          ) : null}
          {usage ? <TokenUsageBadge usage={usage} /> : null}
          {interactive ? (
            <ChevronDown
              className={`workflow-turn-header-chevron ${expanded ? "" : "is-collapsed"}`}
            />
          ) : null}
        </button>
        {modelName ? (
          <span className="workflow-turn-header-model">{modelName}</span>
        ) : null}
      </div>
      <div className="workflow-turn-header-rule" aria-hidden="true" />
    </div>
  );
}
      ...
      <span className="message-token-tooltip">
        ...
        <span className="token-tooltip-rows">
          ...
          {usage.cacheReadInputTokens ? (
            <span className="token-tooltip-row">
              <span>缓存读取</span>
              <strong>{usage.cacheReadInputTokens}</strong>
            </span>
          ) : null}
          <span className="token-tooltip-row">
            <span>总计</span>
