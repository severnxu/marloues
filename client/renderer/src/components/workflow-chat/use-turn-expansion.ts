...
export function useWorkflowTurnExpansion({
  ...
  const previousTurnStateRef = useRef<WorkflowTurnRuntimeState & { id: string }>({
    id: message.id,
  ...
  const setTurnExpanded = useCallback((next: boolean | ((current: boolean) => boolean)) => {
  ...
  useEffect(() => {
    ...
    else if (workflowTurnShouldCollapseAfterRuntime(message, previous, isLastStreaming)) setTurnExpanded(false)
  ...
  }, [defaultExpanded, isLastStreaming, message, message.activity, message.id, message.status, setTurnExpanded])
