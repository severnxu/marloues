...
async function main(): Promise<void> {
  ...
  saveAgentSettings({
    ...
    defaultModel: { providerId: "contract-provider", modelId: "contract-model" },
    mcpServers: [
      {
        id: "contract-mcp",
        name: "Contract MCP",
  ...
  await initRuntime();
  assert(getRuntimeState().activeRuntimeId === "self-built", "initRuntime should honor activeRuntimeId");
  assert(
    getRuntimeState().runtimes.every((runtime) => runtime.status === "available"),
    "all PRD v1 runtime descriptors should be available",
  );

  const runtime = getRuntime();
  const runtimeModels = await runtime.getAvailableModels?.();
  assert(runtimeModels?.some((model) => model.id === "local-loop"), "self-built runtime should expose its local model");
  ...
  assert(listedModels.ok, "endpoint model discovery should call /v1/models successfully");
  assert(
  ...
  assert(profileTest.ok, "endpoint profile test should succeed when /v1/models is reachable");
  ...
  const events = await collect(await runtime.sendMessage({
    threadId: thread.id,
    turnId: "turn-contract",
    content: "hello contract",
    cwd: process.cwd(),
  }));
  assert(events.some((event) => event.kind === "turn-start"), "sendMessage should emit turn-start");
  assert(events.some((event) => event.kind === "text-chunk"), "sendMessage should stream text chunks");
  assert(
    events.some((event) => event.kind === "token-usage" && (event.payload.usage.totalTokens ?? 0) > 0),
    "self-built runtime should emit token usage for completed turns",
  );
  assert(
    events.some((event) => event.kind === "turn-complete" && event.payload.result === "success"),
    "sendMessage should complete successfully",
  );
  const readEvents = await collect(await runtime.sendMessage({
    threadId: thread.id,
  ...
  assert(
    readEvents.some((event) => event.kind === "tool-start" && event.payload.toolName === "self-built.fs.read"),
    "self-built loop should execute read through a tool event",
  );
  assert(textFromEvents(readEvents).includes("contract notes"), "self-built read should stream file content");
  const patchEvents = await collectWithApprovalResponse(
    ...
    (event) => {
      if (event.kind === "approval-request") runtime.respondApproval(event.payload.requestId, true, "once");
    },
  );
  assert(
    patchEvents.some((event) => event.kind === "approval-request" && event.payload.toolName === "self-built.fs.patch"),
    "self-built patch should request approval",
  );
  assert(
    patchEvents.some((event) => event.kind === "tool-complete" && event.payload.isError === false),
    "approved self-built patch should complete tool execution",
  );
  const generatedContent = await import("node:fs").then((fs) => fs.readFileSync(join(loopWorkspace, "generated.md"), "utf-8"));
  assert(generatedContent.includes("created by self-built loop"), "self-built patch should write inside workspace");
  const undoEvents = await collect(await runtime.sendMessage({
    threadId: thread.id,
  ...
  assert(
    undoEvents.some((event) => event.kind === "tool-complete" && String(event.payload.output).includes("removing newly created file")),
    "self-built undo should remove newly created patch files",
  );
  const traversalEvents = await collect(await runtime.sendMessage({
    threadId: thread.id,
  ...
  assert(
    traversalEvents.some((event) => event.kind === "turn-complete" && event.payload.result === "error"),
    "self-built filesystem tools should enforce workspace sandbox",
  );

  const tools = await runtime.listTools();
  assert(tools.some((tool) => tool.name === "contract.echo"), "runtime tools should include configured MCP tools");
  ...
  assert(mcpProbe.ok, "stdio MCP probe should initialize server and list tools");
  assert(mcpProbe.tools.includes("contract.echo"), "stdio MCP probe should discover tools/list result");
  const httpMcpProbe = await probeMcpServer({
    id: "contract-http-mcp",
    name: "Contract HTTP MCP",
    enabled: true,
    config: { type: "http", url: `${remoteMcpServer.baseUrl}/mcp`, headers: { Authorization: "Bearer mcp-key" } },
  });
  assert(httpMcpProbe.ok, "HTTP MCP probe should call initialize and tools/list");
  assert(httpMcpProbe.tools.includes("remote.echo"), "HTTP MCP probe should discover remote tools/list result");
  const sseMcpProbe = await probeMcpServer({
    id: "contract-sse-mcp",
    name: "Contract SSE MCP",
    enabled: true,
    config: { type: "sse", url: `${remoteMcpServer.baseUrl}/sse`, headers: { Authorization: "Bearer mcp-key" } },
  });
  assert(sseMcpProbe.ok, "SSE MCP probe should discover message endpoint and call tools/list");
  assert(sseMcpProbe.tools.includes("remote.echo"), "SSE MCP probe should discover remote tools/list result");
  ...
  const approvalEvents = await collectWithApprovalResponse(approvalStream, (event) => {
    if (event.kind === "approval-request") runtime.respondApproval(event.payload.requestId, true, "once");
  });
  assert(
  ...
  assert(
    approvalEvents.some((event) => event.kind === "tool-start" && event.payload.toolName === "self-built.sensitive-write"),
    "approved flow should start the sensitive tool",
  );
  assert(
    approvalEvents.some((event) => event.kind === "tool-complete" && event.payload.isError === false),
  ...
  const cancelToolEvents = await collectWithApprovalResponse(cancelToolStream, (event) => {
    if (event.kind === "approval-request") runtime.respondApproval(event.payload.requestId, true, "once");
    if (event.kind === "tool-start") void runtime.cancelTool?.(event.payload.toolId);
  });
  assert(
    cancelToolEvents.some((event) => event.kind === "tool-complete" && event.payload.isError),
    "cancelTool should complete the running tool as an error",
  );
  assert(
    cancelToolEvents.some((event) => event.kind === "turn-complete" && event.payload.result === "aborted"),
  ...
  const deniedEvents = await collectWithApprovalResponse(deniedStream, (event) => {
    if (event.kind === "approval-request") runtime.respondApproval(event.payload.requestId, false, "once");
  });
  assert(
    deniedEvents.some((event) => event.kind === "turn-complete" && event.payload.result === "error"),
  ...
  assert(
    interruptedEvents.some((event) => event.kind === "turn-complete" && event.payload.result === "aborted"),
    "interruptTurn should abort the active turn",
  );

  const editableThread = await runtime.createThread("editable");
  await collect(await runtime.sendMessage({
  ...
  assert(truncated?.messages.length === 0, "truncateThread should remove the selected user message and following turns");
  await collect(await runtime.sendMessage({
    threadId: editableThread.id,
  ...
  const editedThread = (await runtime.listThreads()).find((item) => item.id === editableThread.id);
  assert(
    editedThread?.messages.some((message) => message.id === "editable-user-message" && message.content === "edited prompt"),
    "sendMessage should preserve caller-provided messageId for edited prompts",
  );

  const state = await switchRuntime("sdk");
  assert(state.activeRuntimeId === "sdk", "switchRuntime should switch to SDK runtime");
  ...
  setClaudeQueryOverrideForTests((_prompt, options) => {
    const canUseTool = options.canUseTool as ((toolName: string, input: Record<string, unknown>, context: Record<string, unknown>) => Promise<{ behavior: string }>) | undefined;
    async function* stream(): AsyncIterable<unknown> {
      const permission = await canUseTool?.("Bash", { command: "echo sdk approval" }, {
      ...
      yield {
        type: "assistant",
        message: {
          content: permission?.behavior === "allow"
  ...
  const sdkApprovalEvents = await collectWithApprovalResponse(
    ...
    (event) => {
      if (event.kind === "approval-request") sdkRuntime.respondApproval(event.payload.requestId, true, "once");
    },
  );
  assert(
    sdkApprovalEvents.some((event) => event.kind === "approval-request" && event.payload.toolName === "Bash"),
    "SDK runtime should emit approval-request from canUseTool",
  );
  assert(
    sdkApprovalEvents.some((event) => event.kind === "text-chunk" && event.payload.content.includes("sdk approval allowed")),
...
async function startModelServer(): Promise<{ baseUrl: string; close: () => Promise<void> }> {
  ...
  const server = createServer((req, res) => {
    if (req.url === "/v1/models" && req.headers.authorization === "Bearer contract-key") {
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({
        object: "list",
        data: [
          { id: "contract-model-a", object: "model" },
          { id: "contract-model-b", object: "model", display_name: "Contract Model B" },
        ],
      }));
      return;
    }
    // testEndpointModel sends a diagnostic POST /v1/messages ping
    if (req.url === "/v1/messages" && req.method === "POST" && req.headers.authorization === "Bearer contract-key") {
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({
        id: "msg-contract",
        type: "message",
  ...
  assert(address && typeof address === "object", "model test server should listen on a TCP port");
  return {
...
async function startRemoteMcpServer(): Promise<{ baseUrl: string; close: () => Promise<void> }> {
  const { createServer } = await import("node:http");
  const server = createServer(async (req, res) => {
    ...
    if (req.url === "/mcp" && req.method === "POST" && req.headers.authorization === "Bearer mcp-key") {
      ...
      if (message.method === "tools/list") {
        ...
        res.end(JSON.stringify({
          jsonrpc: "2.0",
          id: message.id,
          result: { tools: [{ name: "remote.echo", description: "Remote echo", inputSchema: { type: "object" } }] },
        }));
        return;
      }
    }
  ...
  const address = server.address();
  assert(address && typeof address === "object", "remote MCP test server should listen on a TCP port");
  return {
    baseUrl: `http://127.0.0.1:${address.port}`,
    close: () => new Promise<void>((resolve, reject) => server.close((error) => error ? reject(error) : resolve())),
  };
...
function textFromEvents(events: RuntimeEvent[]): string {
  return events
    .filter((event): event is Extract<RuntimeEvent, { kind: "text-chunk" }> => event.kind === "text-chunk")
