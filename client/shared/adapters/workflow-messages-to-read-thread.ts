...
export type WorkflowTurnItem =
  | Exclude<BaseTurnItem, BaseWebSearchItem | BaseHookPromptItem | BaseMcpToolCallItem | BaseDynamicToolCallItem | BaseReasoningItem>
...
export interface WorkflowMessageBlock {
  ...
  status: Extract<WorkflowTurnStatus, 'running' | 'completed' | 'failed'> | 'running' | 'completed' | 'failed'
...
export function workflowMessagesToWorkflowReadThreadResponse(
  ...
  const chronologicalTurns = workflowMessages
    ...
    .map(workflowBlockToWorkflowTurn)
  ...
  const preview = options.preview ?? latestTurnPreview(latestTurn) ?? latestTurnPreview(firstTurn) ?? ''
  ...
  return {
    ...
    thread: {
      ...
      updatedAt: options.updatedAt ?? latestTurn?.completedAt ?? latestTurn?.startedAt ?? null,
    },
    page: {
      order: 'newest_first',
...
export function workflowReadThreadToWorkflowMessages(readThread: WorkflowReadThreadResponse): WorkflowMessageBlock[] {
...
export function workflowReadThreadTurnsInRenderOrder(readThread: WorkflowReadThreadResponse): WorkflowTurn[] {
  return readThread.page.order === 'newest_first'
    ? [...readThread.turns].reverse()
    : [...readThread.turns]
}

export function workflowTurnToWorkflowMessage(turn: WorkflowTurn): WorkflowMessageBlock {
  const userItems = turn.items.filter((item): item is Extract<WorkflowTurnItem, { type: 'userMessage' }> => item.type === 'userMessage')
  const userContent = userItems.flatMap(item => item.content)
  const items = turn.items.filter((item): item is WorkflowTurnItem => item.type !== 'userMessage')
...
function workflowBlockToWorkflowTurn(block: WorkflowMessageBlock): WorkflowTurn {
  const items: BaseTurnItem[] = block.items.map(workflowItemToWorkflowTurnItem)
  if (block.userContent.length && !items.some(item => item.type === 'userMessage')) {
    items.unshift({
      type: 'userMessage',
...
function workflowItemToWorkflowTurnItem(item: WorkflowTurnItem): BaseTurnItem {
...
function normalizeWorkflowStatus(status: WorkflowTurnStatus): WorkflowMessageBlock['status'] {
  if (status === 'failed') return 'failed'
  if (status === 'completed') return 'completed'
  return 'running'
}

function workflowActivityForTurn(turn: WorkflowTurn): WorkflowMessageBlock['activity'] {
  if (turn.status === 'failed') return 'failed'
  if (turn.status === 'completed') return 'done'
  if (turn.items.some(item => item.type === 'agentMessage' && item.text.trim())) return 'responding'
