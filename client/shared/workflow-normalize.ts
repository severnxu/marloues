...
export function normalizeWorkflowRawEvents(rawEvents: WorkflowRawEvent[]): NormalizedTurn {
  ...
  for (const event of rawEvents) {
    ...
    const normalized = normalizeWorkflowItem(item, params, phase)
...
function ingestProbeResult(items: Map<string, NormalizedThreadItem>, event: WorkflowRawEvent): void {
  const params = asRecord(event.params)
  ...
  for (const candidate of candidates) {
    ...
    for (const item of flattenProbeItems(candidate)) {
      const normalized = normalizeWorkflowItem(item, undefined, 'completed')
      if (!normalized) continue
