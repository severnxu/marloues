...
引导页在开发时直接写代码、未先做设计稿，其样式寄居在 `auth.css` 中，使用 `legacy-tokens.css` 的哈希令牌（`--component-auth-onboarding-*`），与工作台语义令牌脱节。本设计稿用纯语义令牌重建引导页视觉，为后续从 `auth.css` 抽离引导页样式并迁移到语义令牌提供像素基线。
...
引导页真实代码位于 `client/renderer/src/components/onboarding/OnboardingView.tsx`，状态管理于 `client/renderer/src/stores/onboarding-store.ts`。当前样式寄居在 `client/renderer/src/styles/components/auth.css` 的 `onboarding-*` 规则中，使用哈希令牌。后续迁移目标是将引导页样式从 `auth.css` 抽离为独立 CSS，并切换到语义令牌。
