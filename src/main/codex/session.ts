...
export type SessionEvent =
  ...
  | { type: 'turn_started' }
  | { type: 'turn_step_failed'; stepIndex: number; stepType: string; error: string }
  | { type: 'context_compacted'; originalTokens: number; compactedTokens: number }
  | { type: 'raw_event'; event: CodexRawEvent }
  | { type: 'assistant_message'; content: string }
  | { type: 'tool_call'; tool: string; args: Record<string, unknown> }
  | { type: 'tool_result'; tool: string; result: unknown }
  | { type: 'item_event'; phase: 'started' | 'updated' | 'completed'; item: NormalizedThreadItem }
  | { type: 'turn_completed' }
  | { type: 'approval_requested'; id: string; tool: string; toolInput: Record<string, unknown>; threadId: string; cwd?: string }
...
export class CodexAppServerSession {
  ...
  private pendingApprovals: Map<string, { resolve: (value: boolean) => void; reject: (err: Error) => void }> = new Map()

  constructor(sessionId: string, rpc: JsonRpcClient, transport: CodexTransport) {
  ...
  async send(prompt: string): Promise<void> {
    log('[session] send() called, threadId=', this.threadId, 'prompt=', prompt.slice(0, 50))
    ...
    if (!this.threadId) {
      ...
      const result = await this.rpc.request<ThreadStartResult>(
        ClientMethods.ThreadStart,
        {
          ...
          sandbox: settings.sandboxMode || 'read-only',
        }
      )
  ...
  async interrupt(): Promise<void> {
    ...
    await this.rpc.request(ClientMethods.TurnInterrupt, { threadId: this.threadId })
  }

  async respondToApproval(approvalId: string, decision: 'approve' | 'deny', reason?: string): Promise<void> {
    await this.rpc.request(ClientMethods.ApprovalRespond, { id: approvalId, decision, reason })
  ...
  async fork(sourceThreadId: string, cwd?: string): Promise<string> {
    ...
    if (newThreadId) {
      ...
      this.emit({ type: 'thread_forked', threadId: newThreadId, sourceThreadId })
    }
    return newThreadId || ''
  }
  ...
  private async streamUntilTurnComplete(): Promise<void> {
    ...
    while (true) {
      ...
      if (method === 'turn/failed') {
        const err = (params?.error as Record<string, unknown>)?.message as string || 'Turn failed'
        const failedStep = params?.failedStep as Record<string, unknown> | undefined
        if (failedStep) {
          const stepIndex = (failedStep.index as number) || 0
          const stepType = (failedStep.type as string) || 'unknown'
          this.emit({ type: 'turn_step_failed', stepIndex, stepType, error: err })
        }
      ...
      if (method === 'item/started' || method === 'item/updated' || method === 'item/completed') {
        ...
        const phase = method === 'item/started' ? 'started' : method === 'item/updated' ? 'updated' : 'completed'
        ...
        } else if (itemType === 'toolCall' || itemType === 'functionCall' || itemType === 'mcpToolCall') {
          const toolName = (item.name as string) || (item.tool as string) || ''
          const args = (item.arguments as Record<string, unknown>) || {}
          if (method === 'item/started') {
            this.emit({ type: 'tool_call', tool: toolName, args })
          } else if (method === 'item/completed') {
            this.emit({ type: 'tool_result', tool: toolName, result: item.output })
          }
        }
      }
      ...
      if (method === 'context/compacted' || method === 'context_compacted') {
        ...
        this.emit({ type: 'context_compacted', originalTokens, compactedTokens })
      }

      if (method === 'approval/request' || method === 'approval_request') {
        const id = (params?.id as string) || ''
        const tool = (params?.tool as string) || (params?.name as string) || ''
        const toolInput = (params?.toolInput as Record<string, unknown>) || (params?.input as Record<string, unknown>) || {}
        const cwd = params?.cwd as string | undefined
        this.emit({ type: 'approval_requested', id, tool, toolInput, threadId: this.threadId || '', cwd })
      }
    }
  }
  ...
  private handleNotification(method: string, params: unknown): void {
    log('[session] notification received method=', method, 'params=', JSON.stringify(params))
  ...
  private async probeCompletedTurnItems(params: Record<string, unknown>): Promise<void> {
    ...
    for (const attempt of attempts) {
      try {
        ...
        log('[session] turn items probe failed:', attempt.method, JSON.stringify(attempt.params), err)
      }
    }
  }
}
