...
function withAdditionalPathDirs(env: NodeJS.ProcessEnv, pathDirs: string[]): NodeJS.ProcessEnv {
  if (pathDirs.length === 0) return env
  return {
    ...env,
    PATH: [...pathDirs, env.PATH || ''].filter(Boolean).join(getPathSeparator()),
  }
}
...
function buildNativeCodexResolution(vendorRoot: string, targetTriple: string): CodexBinaryResolution {
...
function tryResolvePackageJson(packageName: string, paths: string[]): string | null {
  try {
...
function tryResolveCodexPlatformPackageJson(platformPackageName: string): string | null {
  const codexPackageJsonPath = tryResolvePackageJson('@openai/codex', [process.cwd()])
  if (!codexPackageJsonPath) return null

  try {
    const codexPackageRequire = createRequire(path.join(path.dirname(codexPackageJsonPath), 'bin', 'codex.js'))
    return codexPackageRequire.resolve(`@openai/${platformPackageName}/package.json`)
  } catch {
    return null
  }
}
...
export function resolveBundledCodexBinary(): CodexBinaryResolution | null {
  ...
  const resolvedPackageCandidates = [
    ...
    return [buildNativeCodexResolution(path.join(path.dirname(packageJsonPath), 'vendor'), targetTriple)]
  })

  const nativeCandidates = [
    ...resolvedPackageCandidates,
    ...resourceRoots.flatMap((nodeModulesRoot) => [
      buildNativeCodexResolution(
        path.join(nodeModulesRoot, '@openai', platformPackageName, 'vendor'),
        targetTriple
      ),
      buildNativeCodexResolution(
        path.join(nodeModulesRoot, '@openai', 'codex', 'node_modules', '@openai', platformPackageName, 'vendor'),
        targetTriple
      ),
      buildNativeCodexResolution(
  ...
  const jsCandidates = resourceRoots.map((nodeModulesRoot) => ({
    binaryPath: path.join(nodeModulesRoot, '@openai', 'codex', 'bin', 'codex.js'),
    pathDirs: [],
  }))
...
export class CodexTransportImpl implements CodexTransport {
  ...
  async start(): Promise<void> {
    const { binaryPath, cwd, env, pathDirs = [], args = ['app-server'], onStderr } = this.opts
    ...
    return new Promise((resolve, reject) => {
      ...
      this.child!.stdout.on('data', (chunk: Buffer) => {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            } else if (typeof msg.id === 'number' && msg.id !== 0 && ('result' in msg || 'error' in msg)) {
      ...
      // Wait for process to be ready (give it time to start up)
