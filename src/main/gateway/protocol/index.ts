...
export { OpenAIChatSseParser, parseOpenAIChatSseChunk } from './stream/openai-chat'
export { AnthropicSseFormatter, formatAnthropicSse } from './stream/anthropic-formatter'
export { OpenAIChatSseFormatter, formatOpenAIChatSse } from './stream/openai-chat-formatter'
export { OpenAIResponsesSseFormatter, formatOpenAIResponsesSse } from './stream/responses-formatter'
...
export function decodeRequest(
  ...
  switch (protocol) {
    case 'anthropic':
      return decodeAnthropicRequest(raw as Parameters<typeof decodeAnthropicRequest>[0], requestId)
    case 'openai-chat':
      return decodeOpenAIChatRequest(raw as Parameters<typeof decodeOpenAIChatRequest>[0], requestId)
    case 'openai-responses':
      return decodeOpenAIResponsesRequest(raw as Parameters<typeof decodeOpenAIResponsesRequest>[0], requestId)
  }
}
...
export function parseResponse(
  ...
  switch (protocol) {
    case 'anthropic':
      return parseAnthropicResponse(raw as Parameters<typeof parseAnthropicResponse>[0], requestId, originalModel)
    case 'openai-chat':
    case 'openai-responses':
      return parseOpenAIChatResponse(raw as Parameters<typeof parseOpenAIChatResponse>[0], requestId, originalModel)
  }
}
...
export function needsConversion(sourceProtocol: ProtocolId, targetProtocol: ProtocolId): boolean {
