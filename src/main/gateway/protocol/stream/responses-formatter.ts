...
export class OpenAIResponsesSseFormatter {
  ...
  done(_stopReason = 'stop', usage?: { inputTokens: number; outputTokens: number }): string {
  ...
  private emitPreamble(): string {
    ...
    return (
      this.sse('response.created', { type: 'response.created', response: { ...base } }) +
      this.sse('response.in_progress', { type: 'response.in_progress', response: { ...base } }) +
...
export function formatOpenAIResponsesSse(deltas: IrStreamDelta[], requestId: string, model: string): string {
