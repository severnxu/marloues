...
import type { AgentSettings, AuditEventRecord, ChatSessionRecord, SkillInfo } from "@shared/types";
...
export function GlobalSearchOverlay({
  ...
  const workspaceResults = useMemo(() => {
    ...
    return workspaces.filter((item) => `${item.name} ${item.path}`.toLowerCase().includes(normalizedQuery));
  ...
  const sessionResults = useMemo(() => {
    ...
    return items.sort((a, b) => Number(Boolean(b.isPinned)) - Number(Boolean(a.isPinned)) || b.updatedAt - a.updatedAt);
  ...
  const settingResults = useMemo(() => {
    const items: SearchShortcut[] = [
      ...
      { id: "settings-mcp", title: "MCP", detail: "Server 与工具发现", section: "mcp", icon: "mcp" },
      {
        id: "settings-skills",
        title: "Skills",
      ...
      },
      {
        id: "settings-runtime",
        title: "运行时上下文",
        detail: "当前 Skills、MCP Server、工具快照",
        section: "runtime",
        icon: "runtime",
      },
    ];
  ...
  const providerResults = useMemo(() => {
    ...
    const providerItems: SearchShortcut[] = (agentSettings.providers ?? []).flatMap((provider) => [
      {
  ...
  const mcpResults = useMemo(() => {
    if (!agentSettings) return [];
    const serverItems: SearchShortcut[] = (agentSettings.mcpServers ?? []).map((server) => ({
  ...
  const handleSessionPick = async (session: ChatSessionRecord) => {
    const targetWorkspace = workspaces.find((item) => workspacePathsEqual(item.path, session.workspacePath));
      ...
      >
        ...
        <div className="global-search-results">
          <div className="global-search-section">
            ...
            <ShortcutResults items={settingResults} emptyText="没有匹配的入口" onPick={openSettingsSection} />
          </div>
          <div className="global-search-section">
              ...
              workspaceResults.map((item) => (
                ...
                >
                  <span className="global-search-icon">
                    {item.id === workspace?.id ? <Check size={15} /> : <Folder size={15} />}
                  </span>
                  <span>
            ...
            {sessionResults.length ? (
              sessionResults.map((session) => (
                ...
                >
                  ...
                  <span>
                    <strong>{session.title === "New chat" ? "新对话" : session.title}</strong>
                    <small>
                      {session.workspaceName ? `${session.workspaceName} · ` : ""}
                      {getLastSessionMessage(session) ?? "暂无消息"}
                    </small>
                  </span>
                </button>
              ))
          ...
          <div className="global-search-section">
            ...
            <ShortcutResults
              ...
              onPick={openSettingsSection}
            />
          </div>
          <div className="global-search-section">
            <span className="global-search-section-title">MCP</span>
            <ShortcutResults items={mcpResults} emptyText="没有匹配的 MCP 项" onPick={openSettingsSection} />
          </div>
          <div className="global-search-section">
            <span className="global-search-section-title">Skills</span>
            <ShortcutResults items={skillResults} emptyText="没有匹配的 Skill" onPick={openSettingsSection} />
          </div>
          <div className="global-search-section">
            <span className="global-search-section-title">权限</span>
            <ShortcutResults items={permissionResults} emptyText="没有匹配的权限项" onPick={openSettingsSection} />
          </div>
          <div className="global-search-section">
            <span className="global-search-section-title">审计</span>
            <ShortcutResults items={auditResults} emptyText="没有匹配的审计记录" onPick={openSettingsSection} />
          </div>
        </div>
      </section>
    </div>
...
type SearchShortcutIcon = "settings" | "provider" | "mcp" | "skill" | "permission" | "audit" | "runtime";
...
function ShortcutResults({
  ...
  return (
    <>
      {items.map((item) => (
        <button key={item.id} type="button" className="global-search-result" onClick={() => onPick(item.section)}>
          <span className="global-search-icon">{renderSearchShortcutIcon(item.icon)}</span>
          <span>
...
function filterShortcuts<T extends SearchShortcut>(items: T[], query: string): T[] {
  const sorted = [...items].sort((a, b) => String(a.title).localeCompare(String(b.title)));
  if (!query) return sorted.slice(0, 3);
  return sorted.filter((item) => `${String(item.title)} ${String(item.detail)}`.toLowerCase().includes(query));
}
