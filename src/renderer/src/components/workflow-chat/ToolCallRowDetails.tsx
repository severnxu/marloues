...
export function ToolDetail({
  ...
  const planMarkdown = item.type === 'plan' ? item.text : name === 'plan_snapshot' ? outputText : ''
  const toolSearch = name.includes('tool_search') ? parseToolSearchDetail(input, output) : null
  const webSearch = item.type === 'webSearch' ? parseWebSearchDetail(input, output) : null
  const imageGeneration = item.type === 'imageGeneration' ? parseImageGenerationDetail(input, output) : null
  const usage = name === 'token_count' ? parseUsageDetail(output) : null
  const showDefaultInput = Boolean(input && !planSteps.length && !planMarkdown && !toolSearch && !webSearch && !imageGeneration && !usage)
  const showOutput = Boolean(output && !(name === 'update_plan' && output === 'Plan updated') && !planMarkdown && !toolSearch && !webSearch && !imageGeneration && !usage)
  const running = workflowStatusIsRunning(status)
  const statusText = running ? '运行中' : failed ? '失败' : exitCode !== null ? `exit ${exitCode}` : '成功'
  ...
  return (
    <div className="workflow-tool-card">
      ...
      <div className="workflow-tool-card-body">
        ...
        {planMarkdown ? <MarkdownDetail label="Plan" value={planMarkdown} /> : null}
        {toolSearch ? <ToolSearchDetail data={toolSearch} /> : null}
        {webSearch ? <WebSearchDetail data={webSearch} /> : null}
        {imageGeneration ? <ImageGenerationDetail data={imageGeneration} /> : null}
        {usage ? <UsageDetail data={usage} /> : null}
        {showDefaultInput ? <DetailBlock label={detailInputLabel(item)} value={input} /> : null}
        {showOutput ? <DetailBlock label={failed ? 'Error' : 'Output'} value={output} danger={failed} scrollable /> : null}
        {!planSteps.length && !planMarkdown && !toolSearch && !webSearch && !imageGeneration && !usage && !showDefaultInput && !showOutput ? (
          <div className="workflow-tool-empty">无输出</div>
        ) : null}
      </div>
      <div className={`workflow-tool-status ${failed ? 'danger' : ''}`}>{statusText}</div>
    </div>
  )
}
...
export function ToolIcon({ item }: { item: ToolCallRowItem }) {
  const name = toolName(item)
  if (name === 'update_plan' || name.includes('todo')) return <ListChecks className="h-3.5 w-3.5 flex-shrink-0" />
  if (name === 'token_count') return <Gauge className="h-3.5 w-3.5 flex-shrink-0" />
  if (item.type === 'imageGeneration') return <ImageIcon className="h-3.5 w-3.5 flex-shrink-0" />
  if (isReadToolName(name)) return <FileText className="h-3.5 w-3.5 flex-shrink-0" />
  if (isListToolName(name)) return <FolderTree className="h-3.5 w-3.5 flex-shrink-0" />
  if (isEditToolName(name)) return <FilePenLine className="h-3.5 w-3.5 flex-shrink-0" />
  if (item.type === 'webSearch' || name === 'js' || name.includes('web')) return <Search className="h-3.5 w-3.5 flex-shrink-0" />
  if (isSearchToolName(name)) return <Search className="h-3.5 w-3.5 flex-shrink-0" />
  if (name.includes('shell') || name.includes('command') || name === 'commands') return <SquareTerminal className="h-3.5 w-3.5 flex-shrink-0" />
  if (name.includes('clipboard')) return <Clipboard className="h-3.5 w-3.5 flex-shrink-0" />
...
function codexStyleToolLabel(item: ToolCallRowItem): string | null {
  const name = toolName(item)
  const status = itemStatus(item)
  const verb = status === 'error' || status === 'failed'
    ? '失败：'
  ...
  if (name === 'read' || name === 'read_files' || name.endsWith('.read') || name.endsWith('.read_file')) {
...
function readableToolLabel(item: ToolCallRowItem): string {
  ...
  if (name === 'read' || name === 'read_files' || name.endsWith('.read')) return `已读取 ${toolTargetCount(input) || 1} 个文件`
  if (name === 'workflow_read_file' || name.endsWith('.read_file')) return `已读取 ${basename(firstLine || input || '文件')}`
  if (name === 'glob' || name.endsWith('.glob') || name === 'ls' || name.endsWith('.ls')) return '已列出文件'
  if (name === 'grep' || name.endsWith('.grep')) return '已搜索工作区'
  if (name.includes('tool_search')) return '已搜索工具'
  if (item.type === 'webSearch') return input.includes('"type": "open_page"') ? '已打开页面' : '已搜索网页'
  if (item.type === 'imageGeneration') return itemStatus(item) === 'running' ? '正在生成图片' : '已生成图片'
  ...
  if (isSearchToolName(name) || firstLine.startsWith('rg ')) return firstLine.startsWith('rg --files') ? '已列出项目文件' : '已搜索工作区'

  if (name.includes('shell') || name.includes('command') || name === 'commands') {
    if (/^Get-Content\b/i.test(firstLine)) return `已读取 ${compactCommandTarget(firstLine, 'Get-Content')}`
...
function ToolSearchDetail({ data }: { data: ToolSearchDetailData }) {
  return (
    <div className="workflow-tool-section">
      ...
      {data.limit ? <div className="workflow-tool-muted">Limit {data.limit}</div> : null}
      {data.tools.length ? (
        <div className="workflow-tool-chips">
          {data.tools.map(tool => (
            <span key={tool} className="workflow-tool-chip">{tool}</span>
          ))}
        </div>
      ) : null}
    </div>
...
function WebSearchDetail({ data }: { data: WebSearchDetailData }) {
  ...
  return (
    <div className="workflow-tool-section">
      <div className="workflow-tool-label">{data.type === 'open_page' ? 'Open page' : 'Search'}</div>
      <div className="workflow-tool-primary">{primary}</div>
      {data.queries.length > 1 ? (
        <div className="workflow-tool-list">
          {data.queries.map(query => <div key={query}>{query}</div>)}
        </div>
      ) : null}
    </div>
  )
  ...
  return (
    <div className="workflow-tool-plan">
      {steps.map((step, index) => (
        <div key={`${step.status}-${step.step}-${index}`} className="workflow-tool-plan-row">
          <span className={`workflow-tool-plan-dot ${planStatusTone(step.status)}`}>
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="workflow-tool-muted">{planStatusLabel(step.status)}</div>
          </div>
        </div>
      ))}
    </div>
...
function UsageDetail({ data }: { data: UsageDetailData }) {
  const ratePercent = Math.max(data.primaryPercent ?? 0, data.secondaryPercent ?? 0)

  return (
    <div className="workflow-tool-section">
      <div className="workflow-tool-metrics">
        <UsageMetric label="total" value={formatCompactNumber(data.totalTokens)} />
        <UsageMetric label="last" value={formatCompactNumber(data.lastTokens)} />
        <UsageMetric label="context" value={formatCompactNumber(data.contextWindow)} />
      </div>
      <div className="workflow-tool-usage-bar">
        <span style={{ width: `${Math.min(Math.max(ratePercent, 0), 100)}%` }} />
      </div>
      <div className="workflow-tool-muted">
        {[formatPercent(ratePercent), data.planType].filter(Boolean).join(' / ')}
      </div>
    </div>
  )
}
...
function DetailBlock({ label, value, danger, scrollable }: { label: string; value: string; danger?: boolean; scrollable?: boolean }) {
  ...
  return (
    <div className="workflow-tool-section">
      ...
      <pre className={`workflow-tool-pre ${danger ? 'danger' : ''} ${scrollable ? 'scrollable' : ''}`}>{value}</pre>
    </div>
  )
}
...
function parseToolSearchDetail(input: string, output: string): ToolSearchDetailData | null {
  let query = ''
...
function parseUsageDetail(output: string): UsageDetailData | null {
  ...
  try {
    ...
    return {
      ...
      secondaryPercent: numberFromRecord(parsed, 'rate_limit_secondary_percent'),
...
function detailTitle(item: ToolCallRowItem): string {
  ...
  const firstLine = itemInputText(item).split(/\r?\n/).find(line => line.trim())?.trim()
  if (firstLine && (name.includes('shell') || name.includes('command'))) return compactCommand(firstLine)
